...
module.exports = function (_ref) {
	...
	var BodyVisitor = {
		JSXElement: function JSXElement(path, rewireInformation) {
			...
			if (!(0, _RewireHelpers.wasProcessed)(path) && !t.react.isCompatTag(tagName) && variableBinding !== undefined && variableBinding.scope.block.type === 'Program') {
				...
				if (openingElement.selfClosing) {
					path.replaceWith((0, _RewireHelpers.noRewire)(t.JSXElement(t.JSXOpeningElement(t.JSXIdentifier(componentIdentifier.name), openingElement.attributes, openingElement.selfClosing), undefined, [], true)));
				} else {
		...
		Identifier: function Identifier(path, rewireInformation) {
			...
			var variableBinding = scope.getBinding(variableName);
